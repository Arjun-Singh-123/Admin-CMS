...
cron.schedule("* * * * *", () => {
  ...
  exec(`mkdir  ${backupDir}`, (err) => {
    ...
    const connectionString = `postgresql://postgres.lpflijrmgfjwrcvpdvpi:!Appteg@123!@aws-0-ap-south-1.pooler.supabase.com:6543/postgres`;

    // Execute the pg_dump command
    const command = `pg_dump "${connectionString}" -f "${backupFile}"`;
